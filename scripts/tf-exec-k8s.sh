...
# Add the following provisioner block to your Terraform file:
...
provisioner "remote-exec" {
  inline = [
    ...
    "echo '${aws_instance.master.private_ip}' >> hosts",
    "echo '[kube-worker]' >> hosts",
    "for i in ${aws_instance.worker.*.private_ip}; do echo $i >> hosts; done",
    "ansible-playbook -i hosts install-kubernetes.yaml"
  ]
}

# provisioner block will install Ansible on the EC2 instances and run the install-kubernetes.yaml playbook using the ansible-playbook command.

# Update the aws_instance resource block to include the provisioner block:
...
# provisioner block is added to both the aws_instance.master and aws_instance.worker resource blocks.

replace install-kubernetes.yaml with the path to actual playbook.